...
class Mobile_WS_RelatedRecordsWithGrouping extends Mobile_WS_QueryWithGrouping {
	...
	function process(Mobile_API_Request $request) {
		...
		$relatedmodule = $rest_data['relatedmodule'];
		...
		if ($functionHandler) {
			...
			$fieldnames = Mobile_WS_Utils::getEntityFieldnames($relatedmodule);
			foreach($fieldnames as $fieldname) {
				$selectColumnClause .= sprintf("%s,", $fieldname);
			}
			$selectColumnClause = rtrim($selectColumnClause, ',');
			if(count($relatedRecords)==0){
				$result = array('records' => array(), 'nextPage' => 0 );
				$response = new Mobile_API_Response();
				$response->setResult($result);
				return $response;
			}
			// Perform query to get record information with grouping
			$wsquery = sprintf("SELECT $selectColumnClause FROM %s WHERE id IN ('%s') ORDER BY modifiedtime desc LIMIT 0,50;", $relatedmodule, implode("','", $relatedRecords));
