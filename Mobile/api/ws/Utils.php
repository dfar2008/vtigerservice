...
//		$result = $adb->pquery(
//			"SELECT fieldname FROM vtiger_field INNER JOIN
...
//		}
		$fieldnames = array();
		switch($module) {
			case 'Accounts': $fieldnames = array('id','accountname','rating','phone'); break;

			case 'Contacts': $fieldnames = array('id','salutationtype','firstname','lastname','phone','account_id'); break;
			case 'Potentials': $fieldnames = array('id','potentialname','sales_stage','closingdate','related_to','probability','forecast_amount'); break;
		...
		while($resultrow = $adb->fetch_array($result)) {
			...
			$fieldgroups[$blocklabel][$resultrow['fieldname']] = 
				array(
					...
					'mandatory' => self::fixMandatory( $resultrow['typeofdata'])
				);
		}
	...
	static function fixMandatory($typeofdata) {
		list($type,$mandatory)= explode('~',$typeofdata);
		return $mandatory=='M' ? 1:0;
	}
