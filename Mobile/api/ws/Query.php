...
class Mobile_WS_Query extends Mobile_WS_FetchRecordWithGrouping {
	...
	function process(Mobile_API_Request $request) {
		...
		$nolabel = $request->get('nolabel', false);
		$nextPage = 0;
		$queryResult = false;
		...
		$theMethod = $nolabel? 'processQueryResultRecordnoLabel':'processQueryResultRecord';
		$records = array();
		if (!empty($queryResult)) {
			foreach($queryResult as $recordValues) {
				$records[] = $this->$theMethod($recordValues, $current_user);
			}
		}
